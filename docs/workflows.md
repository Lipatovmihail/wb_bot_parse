...
- **Генерация чанков дат** (для каждого артикула):
...
- **Оптимизация сбора данных**:
  1. Собираются все чанки для всех артикулов селлера
  2. Определяется общий период: `seller_min_date_from` (min всех `date_from`) и `seller_max_date_to` (max всех `date_to`)
  3. **Один запрос по селлеру** за весь период (`TOTAL_NMID_FETCH_BY_SELLER_SQL`) — выгружаются все заказы всех артикулов селлера
  4. Данные группируются по `nmID` в Python
  5. Для каждого артикула данные фильтруются по его чанкам дат в Python
...
13. **TOTAL воркфлоу: пул артикулов**
    ...
    - Один и тот же пул используется во всех четырёх циклах (day_metrics, stock_history, stock_today, funnel_metrics)
    - Это гарантирует, что все этапы обрабатывают одинаковый набор артикулов

14. **TOTAL воркфлоу: оптимизация сбора данных для day_metrics**
    - **Один запрос по селлеру** за весь период (все артикулы сразу) вместо множества запросов по каждому чанку
    - Определяется общий период: `seller_min_date_from` (min всех `date_from`) и `seller_max_date_to` (max всех `date_to`)
    - Данные группируются по `nmID` в Python, затем для каждого артикула фильтруются по его чанкам дат
